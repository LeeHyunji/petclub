	...
	var welUploadInputBox = $Element("imageFile");
	...
	var nowUrl = location.protocol + "//" + location.host  + locationPathname;
		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false; // HTML5 부분 처리시 까지 False로... 
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function drop(ev) {
		...
		if (nImageFileCount >= 10){
			alert("UpLoad Error[Count] : 최대 10장까지만 등록할 수 있습니다.");
			return;
		}
		
		if(typeof ev.dataTransfer.files == 'undefined'){
			alert("UpLoad Error[HTML5] : HTML5 지원이 정상적으로 이루어지지 않는 브라우저입니다.");
		}else{
			//변수 선언
			var wel,
				files,
			...
			if (!!files && nCount === 0){
				...
				alert("UpLoad Error[Drag] : 정상적인 첨부방식이 아닙니다.");
				return ;
			}
			
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("UpLoad Error [Type] : 이미지파일 (jpg,gif,png,bmp)만 업로드 가능합니다.");
				} else if(files[i].size > nMaxImageSize){
					alert("UpLoad Error [Max Byte]:이미지 용량이 10MB를 초과하여 등록할 수 없습니다.");
				} else {
					//제한된 수만 업로드 가능.
					if ( j < nMaxImageCount ){
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("UpLoad Error [All Max Bytes]전체 이미지 용량이 50MB를 초과하여 등록할 수 없습니다. \n\n (파일명 : "+sFileName+", 사이즈 : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
    ...
    function html5Upload() {	
    	...
    	sUploadURL= nowUrl + '/resources/commons/SmartEditiorFileUploader_html5.do'; 	//upload URL
	...
	function onAjaxError (){
		alert("UpLoad Error [URL] : [가이드]사진 업로더할 서버URL셋팅이 필요합니다.-onAjaxError"); //설치 가이드 안내 문구임. 실 서비스에서는 삭제. 
	}
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("imageFile"),{
 			sUrl  : '/common/imageupload.do',
 	        sCallback : '/resources/smarteditor/quick_photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {
 	    		...
 	    		aResult[0].sAlt = imageAlt._element.value;
