...
var MarkerOverlappingRecognizer = function(opts) {
	this._options = jQuery.extend({
		...
		intersectNoticeTemplate: '<div class="marker-overlap"><span>{{count}}</span>개의 마커가 있습니다.</div>',
		intersectList: true,
		intersectListTemplate: '<div class="marker-overlap-template">'
		+ '<ol>'
		+ '{{#repeat}}'
	...
	this._overlapInfoEl = jQuery('<div style="position:absolute;z-index:100;margin:0;padding:0;display:none;"></div>');
	this._overlapListEl = jQuery('<div style="position:absolute;z-index:100;margin:0;padding:0;display:none;"></div>');
};
...
MarkerOverlappingRecognizer.prototype = {
	...
	setMap: function(map) {
		...
		if (map) {
			...
			this._overlapListEl.on('mousewheel DOMMouseScroll mousedown', function(e) {
				e.stopPropagation();
			});
		}
	...
	_bindEvent: function(map) {
		...
		listeners.push(
			map.addListener('idle', jQuery.proxy(this._onIdle, this)),
			map.addListener('click', jQuery.proxy(this._onIdle, this))
		);
	...
	_bindMarkerEvent: function(marker) {
		marker.__intersectListeners = [
			marker.addListener('mouseover', jQuery.proxy(this._onOver, this)),
			marker.addListener('mouseout', jQuery.proxy(this._onOut, this)),
			marker.addListener('mousedown', jQuery.proxy(this._onDown, this))
		];
	...
	_onOver: function(e) {
		...
		if (overlaped.length > 1) {
			...
			if (this._options.intersectNotice) {
				this._overlapInfoEl.empty().append(this._options.intersectNoticeTemplate.replace(/\{\{count\}\}/g, overlaped.length));
				this._overlapInfoEl.css({
					left: 0
					, top: -10000
				}).show();

				var width = this._overlapInfoEl.outerWidth();
	...
	_renderIntersectList: function(overlaped, offset) {
		...
		if (matches && matches.length >= 2) {
			...
			listLayer.append(jQuery(template));

			var placeholder = jQuery('#intersects-'+ uid);
		...
		for (var i=0, ii=overlaped.length; i<ii; i++) {
			var c = overlaped[i],
				item = jQuery(itemTemplate.replace(/\{\{(\w+)\}\}/g, function(match, symbol) {
					if (symbol === 'order') {
						return i+1;
			...
			item.on('click', jQuery.proxy(self._onClickItem, self, c.marker));
			items.push(item);
		};
		...
		var width = listLayer.outerWidth();
